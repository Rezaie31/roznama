...
let tasks = [];
let folders = [];
let todayProgressMap = {}; // taskId -> 0..100 (برای «امروز» واقعی، صرف نظر از تاریخ انتخابی نمودار روزانه)
let logsCache = []; // {date, taskId, progress} همه‌ی امسال تا امروز
let cachedYear = logicalNow().getFullYear();
...
async function init() {
  ...
  folders.forEach((f) => openFolders.add(f.folderId));
...
function currentDayPickerValue() {
  const el = document.getElementById("day-picker");
  return el ? el.value : dateKey(0);
}
...
async function setProgress(taskId, value) {
  ...
  renderTasks();
  renderProgress();
  renderDailyChart(currentDayPickerValue());
  renderWeeklyChart();
  renderMonthlyChart();
  renderYearlyChart();
...
// ===================== PROGRESS BAR + RING (مخصوص امروز واقعی) =====================
...
// ===================== DAILY CHART (قابل انتخاب هر روز دلخواه) =====================
async function renderDailyChart(dateStr) {
  dateStr = dateStr || dateKey(0);
  ...
  if (year === cachedYear) {
    ...
    const res = await apiCall("getLogs", { username, startDate: dateStr, endDate: dateStr });
    logs = res.ok ? res.logs : [];
  }
  const progMap = {};
  logs.filter((l) => l.date === dateStr).forEach((l) => (progMap[l.taskId] = l.progress));

  const ctx = document.getElementById("daily-chart");
  const labels = tasks.map((t) => t.taskName);
  const data = tasks.map((t) => progMap[t.taskId] || 0);

  if (dailyChart) dailyChart.destroy();
  dailyChart = new Chart(ctx, {
    type: "bar",
    data: { labels, datasets: [{ data, backgroundColor: data.map(colorForPct), borderRadius: 6 }] },
    options: {
      responsive: true,
...
function initPickers() {
  ...
  dayPicker.max = todayStr;
  dayPicker.addEventListener("change", () => renderDailyChart(dayPicker.value));
...
// ===================== JOURNAL: TOP REMINDER (قبول/رد کردن برنامه‌ی امروز) =====================
...
// ===================== JOURNAL: BOTTOM COMPLETION CHECK (آیا دیروز رو انجام دادی؟) =====================
function renderCompletionCheck(yesterdayNote, yesterdayCompleted) {
  const box = document.getElementById("completion-check");
  const textEl = document.getElementById("completion-note-text");
  ...
  if (yesterdayCompleted === "done" || yesterdayCompleted === "not_done") {
    actions.style.display = "none";
    resultEl.style.display = "block";
    resultEl.textContent = yesterdayCompleted === "done" ? "دیروز رو کامل کردی! 🎉" : "اشکال نداره، جبرانش می‌کنی 💪";
  } else {
    actions.style.display = "flex";
    resultEl.style.display = "none";
  }
...
async function respondCompletion(choice) {
  await apiCall("saveNoteCompletion", { username, date: dateKey(-1), completed: choice });
  document.getElementById("completion-actions").style.display = "none";
  const resultEl = document.getElementById("completion-result");
  resultEl.style.display = "block";

  if (choice === "done") {
    resultEl.textContent = "دیروز رو کامل کردی! 🎉";
    launchConfetti();
    showToast("آفرین! همینطوری ادامه بده 🎉", "success");
  } else {
    resultEl.textContent = "اشکال نداره، جبرانش می‌کنی 💪";
    showToast("مهم نیست، امروز جبران کن");
  }
}
...
async function openMyNotes() {
  ...
  const res = await apiCall("getAllNotes", { username });
  ...
  list.innerHTML = notes
    ...
    <div class="note-entry">
      <div class="note-entry-head">
        ...
        ${n.timestamp ? `<span class="mono note-time">${formatTimeFromIso(n.timestamp)}</span>` : ""}
      </div>
      <div class="note-entry-text">${escapeHtml(n.note)}</div>
    </div>`
    )
    .join("");
}
